...
public class StudentsQuestion1 {
	...
	public static void main(String[] args) {
		...
		System.out.println(words.stream().collect(groupingBy(s -> s, counting())).entrySet().stream()
				.max(((o1, o2) -> o1.getValue() > o2.getValue() ? 1 : -1)).get().getKey());

		System.out.println(words.stream().collect(groupingBy(s -> s, counting())).entrySet().stream()
				.min(((o1, o2) -> o1.getValue() > o2.getValue() ? 1 : -1)).get().getKey());

		Map<String, Long> frequentChars = words.stream().collect(groupingBy(Function.identity(), counting()));
